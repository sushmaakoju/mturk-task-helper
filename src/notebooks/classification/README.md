...
### Choice of Data representation, formats and reason for choosing the preferred data types
    ...
    4. For confusion matrix, a more elaborate statistics library that has been used: PyCM, a Multi-class confusion matrix library, https://github.com/sepandhaghighi/pycm

### Before running any of following Notebooks/steps, please follow instructions below to install required packages:

Clone the repository:
```
git clone https://github.com/sushmaakoju/mturk-task-helper.git
cd mturk-task-helper
```

#### Preparation to Start Notebooks:

Assuming you already installed Python3.8.x, or Anaconda3 is configured 
...
- Make sure you have command prompt from cloned repository:
    ...
    or
    ```
    cd cd mturk-task-helper-master
    ```

- Install the requirements before running any of notebooks:
...
- To run notebooks in Classification:

    * Install Jupyter Lab. If you do not have Jupyter lab installed already, please follow instructions from [](https://jupyterlab.readthedocs.io/en/stable/getting_started/installation.html)
    * Go to Command prompt/terminal and type following:
        ```
        jupyter lab
        ```
        or with a custom port as follows:
        ```
        jupyter lab --port 8889
        ```
    * Launch http://localhost:XXXX/lab (XXXX refers to port number whether default or the one you entered from above step)
    * Navigate to src/notebooks/classification folder from Jupyter Notebook
...
8. Create Batch of HIT tasks for Classification tasks:
    * This section is expected to create a batch of scripts, given a set of image urls hosted on a server.
...
11. How to evaluate results:
    ...
    * This is shared individually with the team so the folders, csv files are not part of this
        ...
        1. Analyze received batch results file and its contents. For each worker, extract labels and compare with answers. 
            ...
            * [classification_task_scoring](https://github.com/sushmaakoju/mturk-task-helper/blob/main/src/notebooks/classification/classification_task_scoring.ipynb)<br>
        ...
        2. We need to find workerwise agreements, Cohen Kappa score is statistic that is used to measure inter-rater reliability.
            * [Cohen Kappa Score](https://en.wikipedia.org/wiki/Cohen%27s_kappa) <br>
            * [cohen_kappa_score](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.cohen_kappa_score.html)

        2. Another goal of executing a small batch of HIT tasks for classification is to check how many workers would be enough to get best accuracy. We need to identify voted accuracy of workers. There are two voted accuracy groups to consider: a) 5 choose 3 workers and b) 5 workers. Invidual code-level comments are available in the notebooks.
